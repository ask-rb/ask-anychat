...
Patch numbers advance by exactly one per release: `0.2.0` → `0.2.1` →
`0.2.2`. Never skip or jump patch numbers (`0.2.0` → `0.2.3` is wrong), even
...
gemchain guard ask-anychat
gemchain update ask-anychat <new-version> --dry-run
gemchain update ask-anychat <new-version>
```
...
3. Build passes — `bundle exec rake build` (or `gem build ask-anychat.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/anychat/version.rb` by gemchain, not by hand.
...
| `lib/ask/anychat/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.2.0` → `0.2.1`.
- Several bug fixes ship together: one release, `0.2.0` → `0.2.1` (never
  `0.2.3`).
- New backwards-compatible API: `0.2.1` → `0.3.0`.
...
- This gem bumped because a dependency released: patch only, e.g. `0.2.1` →
  `0.2.2`, via the gemchain cascade — never hand-published.
